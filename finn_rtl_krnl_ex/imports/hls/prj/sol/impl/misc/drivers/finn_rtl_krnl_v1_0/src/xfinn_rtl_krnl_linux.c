...
typedef struct {
    ...
    XFinn_rtl_krnl_uio_map maps[ MAX_UIO_MAPS ];
} XFinn_rtl_krnl_uio_info;

/***************** Variable Definitions **************************************/
static XFinn_rtl_krnl_uio_info uio_info;
...
static int uio_info_read_name(XFinn_rtl_krnl_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XFinn_rtl_krnl_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XFinn_rtl_krnl_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XFinn_rtl_krnl_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XFinn_rtl_krnl_Initialize(XFinn_rtl_krnl *InstancePtr, const char* InstanceName) {
	XFinn_rtl_krnl_uio_info *InfoPtr = &uio_info;
