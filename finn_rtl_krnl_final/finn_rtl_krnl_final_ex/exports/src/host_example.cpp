...
int main(int argc, char** argv)
{
    ...
    //
    kernel = clCreateKernel(program, "finn_rtl_krnl_final", &err);
    ...
    ext_stream.flags = 0; // Argument ID of kernel AXI4-Stream master interface axis00 (write_only.)
    axis00_stream = clCreateStream(device_id, CL_STREAM_READ_ONLY, CL_STREAM, &ext_stream, &err);
    if (err != CL_SUCCESS) {
        printf("ERROR: Failed to create stream (clCreateStream) for kernel interface axis00 (arg 0): %d\n", err);
        printf("ERROR: Test failed\n");
        exit(1);
    }

    ext_stream.flags = 1; // Argument ID of kernel AXI4-Stream slave interface axis01 (read_only.)
    axis01_stream = clCreateStream(device_id, CL_STREAM_WRITE_ONLY, CL_STREAM, &ext_stream, &err);
    if (err != CL_SUCCESS) {
        printf("ERROR: Failed to create stream (clCreateStream) for kernel interface axis01 (arg 1): %d\n", err);
        printf("ERROR: Test failed\n");
        exit(1);
    }
    ...
    for (cl_uint i = 0; i < number_of_words*1; i++) {
        if ((cl_uint)(h_data[i] + 1) != read_data[i]) {
            printf("ERROR in finn_rtl_krnl_final::axis00_stream - array index %d - input=%d (0x%x), output=%d (0x%x)\n", i, h_data[i], h_data[i], read_data[i], read_data[i]);
            check_status = 1;
        }
    }
